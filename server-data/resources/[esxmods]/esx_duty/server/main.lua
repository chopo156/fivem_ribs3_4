...
AddEventHandler('duty:onoff', function(job)
    ...
    elseif job == 'offambulance' then
        ...
        TriggerClientEvent('chat:addMessage', -1, { template = '<div class="chat-message system"><b>Services {0} : </b> {1}</div>', args = { fal, "is now On Duty as EMS" } })
    elseif job == 'offmecano' then
        xPlayer.setJob('mecano', grade)
        TriggerClientEvent('esx:showNotification', _source, _U('onduty'))

        TriggerClientEvent('chat:addMessage', -1, { template = '<div class="chat-message system"><b>Services {0} : </b> {1}</div>', args = { fal, "is now On Duty as Mechanic" } })
    end
