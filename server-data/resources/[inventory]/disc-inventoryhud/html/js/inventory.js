...
//1.envanter
var envanterAdi = null;
var maxWeight = 120;
var getWeight = 0;
var playerId = null;

//2.envanter
var envanterAdi2 = null;
var maxWeight2 = 10;
var getWeight2 = 0;
        ...
        if(data.characters !== null) {
            $.each(data.characters, function (index, char) {
                if (char.charid !== 0) {
                    ...
                    $('[data-charid=' + charid + ']').html('<h2 class="character-fullname">'+ char.firstname +' '+ char.lastname +'</h2><div class="character-info"><p class="character-info-work"><strong>Job : </strong><span>'+ char.job +'</span></p><p class="character-info-money"><strong>Money : </strong><span>'+ char.money + ' ₺' +'</span></p><p class="character-info-bank"><strong>Bank : </strong><span>'+ char.bank + ' ₺' +'</span></p> <p class="character-info-dateofbirth"><strong>D.O.B : </strong><span>'+ char.dateofbirth +'</span></p> <p class="character-info-gender"><strong>Gender : </strong><span>'+ char.sex +'</span></p> <p class="character-info-height"><strong>Height : </strong><span>'+ char.height +'</span></p> <p class="character-info-phone-number"><strong>Phone Number : </strong><span>'+ char.phone_number   +'</span></p> </div>').attr("data-ischar", "true");
                }
            });
        }
    };
...
window.addEventListener("message", function (event) {
    if (event.data.action == "display") {
        ...
        envanterAdi = event.data.envanterAdi;
        playerId = event.data.playerId;
        getWeight = event.data.getWeight;
    ...
    if ($('#search').val() !== '') {
        ...
        envanterAdi2 = event.data.envanterAdi2;
        getWeight2 = event.data.getWeight2;
        maxWeight2 = event.data.maxWeight2;
...
function canCarryItemSwap(firstItem, lastItem, invTier, invOwner) {

    var ilkItemToplamAgirlik = null;
    var ikinciItemToplamAgirlik = null;

    if (invOwner == originOwner) {
        ikinciItemToplamAgirlik = firstItem.weight * firstItem.qty;
        ilkItemToplamAgirlik = lastItem.weight * lastItem.qty;
    } else {
        ilkItemToplamAgirlik = firstItem.weight * firstItem.qty;
        ikinciItemToplamAgirlik = lastItem.weight * lastItem.qty;
    }

    var AgirlikEksiIlkItem = getWeight - ilkItemToplamAgirlik;
    var ikinciEnvanterKalanAgirlik = getWeight2 - ikinciItemToplamAgirlik;
    var ikinciEnvanterItemEkle = ikinciEnvanterKalanAgirlik + ilkItemToplamAgirlik;

    var AgirlikEksiIlkItem = getWeight - ilkItemToplamAgirlik;
    var weightWithTestItem = AgirlikEksiIlkItem + ikinciItemToplamAgirlik;

    if (weightWithTestItem <= maxWeight) {
        if (ikinciEnvanterItemEkle <= maxWeight2) {
            return true;
        } else {
            InventoryLog('You cant put that much weight!.')
        }
    } else {
        InventoryLog('You cant carry more weight.')
        return false;
    }
}

function canCarryItem(item, getWeight, maxWeight) {
    var itemWeight = item.weight * item.qty;
    var kalanAgirlik = maxWeight - getWeight;

    if (itemWeight <= kalanAgirlik) {
        return true;
    } else {
        return false;
    }
...
function canCarryItemMove(item, getWeight, maxWeight, moveQty) {
    var itemWeight = item.weight * moveQty;
    var kalanAgirlik = maxWeight - getWeight;
...
function inventorySetup(invOwner, items, money, invTier) {
    setupPlayerSlots();
    $('#player-inv-label').html(firstTier.label);     // $('#player-inv-label').html(char.firstname +' '+ char.lastname + ' [' + charid + ']');
    ...
    $.each(items, function (index, item) {
        getWeight = (item.weight * item.qty) + getWeight;
...
function secondInventorySetup(invOwner, items, invTier, money) {
    ...
    envanterAdi2 = secondTier.label;
    $('#other-inv-label').html(envanterAdi2);
    ...
    $('#second-cash').html('$' + formatCurrency(money.cash));
    ...
    $.each(items, function (index, item) {
        getWeight2 = (item.weight * item.qty) + getWeight2;
    ...
    $('#other-used').html(getWeight2);
}

function setupPlayerSlots() {
    // var maxWeight = firstTier.slots
    $('#inventoryOne').html("");
    $('#player-inv-id').html("");
...
$(document).ready(function () {
    ...
    $("#take").mouseenter(function () {
        ...
        $('.near-players-wrapper').find('.popup-body').html('');
        $('.near-players-wrapper').find('.popup-body').html('');
        $('.near-players-list .popup-body').append(`<div class="cashtake" data-id="cash">Nearby Players</div>`);
    ...
    $("#store").mouseenter(function () {
        ...
        $('.near-players-wrapper').find('.popup-body').html('');
        $('.near-players-wrapper').find('.popup-body').html('');
        $('.near-players-list .popup-body').append(`<div class="cashstore" data-id="cash">Nearby Players</div>`);
    ...
    $("#pay").mouseenter(function () {
        ...
        successAudio.play();
        $('.near-players-wrapper').find('.popup-body').html('');
        $('.near-players-list .popup-body').append(`<div class="cashchoice" data-id="cash">Nearby Players</div>`);
    ...
    $('#inventoryOne, #inventoryTwo').on('mouseenter', '.slot', function () {
        ...
        if (itemData !== undefined) {
            ...
            if (itemData.description !== undefined) {
                $('.tooltip-div').find('.tooltip-desc').html('Weight: ' + itemData.weight + ' lbs<br>' + 'Total Weight: ' + itemData.qty * itemData.weight + ' lbs');
            } else {
                $('.tooltip-div').find('.tooltip-desc').html('Weight: ' + itemData.weight + ' lbs<br>' + 'Total Weight: ' + itemData.qty * itemData.weight + ' lbs');
            }

            if (itemData.weight !== undefined) {
                $('.tooltip-div').find('.tooltip-weight').html('Weight: ' + itemData.weight + ' lbs<br>' + 'Total Weight: ' + itemData.qty * itemData.weight + ' lbs');
            } else {
...
function AttemptDropInEmptySlot(origin,  destination, moveQty) {
    ...
    if (result === -1) {
        ...
        if (moveQty === item.qty) {
            ...
            var canCarryItem1 = true;
            var destInv = getWeight2;
            var weightSettings = destination.parent().data('invTier').slots;
            if (destination.parent().data('invOwner') == originOwner && destination.parent().data('invTier').name == 'player') {
                destInv = getWeight;
                weightSettings = 120;
            }

            if (origin.data('invOwner') != destination.parent().data('invOwner')) {
                var canCarryItem1 = canCarryItem(item, destInv, weightSettings)
            } else if (origin.data('inventory') != destination.data('inventory')) {
                var canCarryItem1 = canCarryItem(item, destInv, weightSettings)
            }   
            ...
            if (canCarryItem1) {
                ...
                InventoryLog('Maximum Weight Reached!')
            }
        } else {
            var canCarryItem1 = true;
            var destInv = getWeight2;
            ...
            if (origin.data('invOwner') != destination.parent().data('invOwner')) {
                var canCarryItem1 = canCarryItemMove(item, destInv, weightSettings, moveQty)
            } else if (origin.data('inventory') != destination.data('inventory')) {
                var canCarryItem1 = canCarryItemMove(item, destInv, weightSettings, moveQty)
            }
        ...
        if (result === 1) {
            ...
            InventoryLog("Target Inventory Owner Undefined");
        }
    }
}
    ...
    if (result === -1) {
        ...
        if (originItem.itemId === destinationItem.itemId && destinationItem.stackable) {
            ...
            if (moveQty != originItem.qty && destinationItem.qty + moveQty <= destinationItem.max) {
                ...
                if (origin.data('invOwner') != destination.parent().data('invOwner')) {
                    var canCarryItem1 = canCarryItemMove(originItem, destInv1, weightSettings, moveQty)
                } else if (origin.data('inventory') != destination.parent().data('inventory')) {
                    var canCarryItem1 = canCarryItemMove(originItem, destInv1, weightSettings, moveQty)
                }
                ...
                if (destinationItem.qty === destinationItem.max) {
                    ...
                    if (origin.data('invOwner') != destination.parent().data('invOwner')) {
                        var canCarryItem1 = canCarryItem(destinationItem, destInv1, weightSettings)
                    } else if (origin.data('inventory') != destination.data('inventory')) {
                        var canCarryItem1 = canCarryItem(destinationItem, destInv1, weightSettings)
                    }
                    ...
                    if (origin.data('invOwner') != destination.parent().data('invOwner')) {
                        var canCarryItem1 = canCarryItem(originItem, destInv1, weightSettings)
                    } else if (origin.data('inventory') != destination.data('inventory')) {
                        var canCarryItem1 = canCarryItem(originItem, destInv1, weightSettings)
                    }
            ...
            if (origin.data('invOwner') != destination.parent().data('invOwner')) {
                var canCarryItem1 = canCarryItemSwap(originItem, destinationItem, destination.parent().data('invTier').name, destination.parent().data('invOwner'))
            } else if (origin.data('inventory') != destination.data('inventory')) {
                var canCarryItem1 = canCarryItemSwap(originItem, destinationItem, destination.parent().data('invTier').name, destination.parent().data('invOwner'))
            }
...
function AddItemToSlot(slot, data) {
    ...
    slot.find('.item-count').html(data.qty + " ( weight: " + data.weight * data.qty + ")");
...
function F8Log(log, bool) {
    if (bool == null) {
        bool = false
    }
    // $('.inv-log').html(log + "<br>" + $('.inv-log').html());
    $.post("http://disc-inventoryhud/F8Log", JSON.stringify({
        log: log,
        bool: bool
    }));
}
