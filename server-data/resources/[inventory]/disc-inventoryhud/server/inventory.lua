...
AddEventHandler("disc-inventoryhud:GiveItem", function(data)
    ...
    if targetPlayer.canCarryItem(data.item.id, data.count) then
      ...
      TriggerClientEvent('disc-inventoryhud:refreshInventory', data.target)
        ...
        TriggerEvent('mythic_notify:client:SendAlert', targetPlayer.source, { type = 'error', text = 'You cant carry that much weight!' })
        TriggerEvent('mythic_notify:client:SendAlert', sourcePlayer.source, { type = 'error', text = ''.. targetPlayer.firstname .. 'cant carry that much weight!' })
    end
end)
...
function AttemptMerge(item, inventory, count)
    local max = getItemDataProperty(item.name, 'max') or 10000000000
    for k, v in pairs(inventory) do
        if v.name == item.name then
...
function AddToEmpty(item, type, inventory, count)
    local max = getItemDataProperty(item.name, 'max') or 1000000000
